...
static dpp::emoji find_emoji(std::string name) {
	...
	else throw std::exception("emoji not found");
}
...
dpp::task<void> blackjack::play(MYSQL* db, const dpp::slashcommand_t& event) {
	...
	if (balance == -1) {
		event.co_edit_original_response(no_reg);
	}
	if (balance < amount) {
		event.co_edit_original_response(dpp::message("bạn không đủ số dư"));
		co_return;
	}
	distribute();
	...
	while (true) {
		...
		event.from->creator->on_button_click.when([this](const dpp::button_click_t& b) { // Button clicked
			if (user_id == b.command.usr.id && b.custom_id == std::to_string(user_id) + "_h") 
			{
		...
		event.from->creator->on_button_click.when([this](const dpp::button_click_t& b) { // Button clicked
			if (user_id == b.command.usr.id && b.custom_id == std::to_string(user_id) + "_s") 
			{
				b.reply(dpp::ir_deferred_update_message, "");
				return true;
			}
